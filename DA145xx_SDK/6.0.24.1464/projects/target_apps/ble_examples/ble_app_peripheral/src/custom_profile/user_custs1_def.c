...
const struct attm_desc_128 custs1_att_db[CUSTS1_IDX_NB] =
{
    ...
    // Control Point Characteristic Value (Supports multi-byte writes up to 20 bytes)
    [SVC1_IDX_CONTROL_POINT_VAL]       = {SVC1_CTRL_POINT_UUID_128, ATT_UUID_128_LEN, PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE),
    ...
    // LED State Characteristic Value (Supports multi-byte writes up to 20 bytes)
    [SVC1_IDX_LED_STATE_VAL]           = {SVC1_LED_STATE_UUID_128, ATT_UUID_128_LEN, PERM(WR, ENABLE) | PERM(WRITE_COMMAND, ENABLE) | PERM(WRITE_REQ, ENABLE),
    ...
    // Long Value Characteristic Value (Supports multi-byte writes up to 50 bytes)
    [SVC1_IDX_LONG_VALUE_VAL]          = {SVC1_LONG_VALUE_UUID_128, ATT_UUID_128_LEN, PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(NTF, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE),
