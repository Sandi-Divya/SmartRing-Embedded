/**
 ****************************************************************************************
 *
 * @file user_custs1_def.h
 *
 * @brief Custom Server 1 (CUSTS1) profile database definitions.
 *
 ****************************************************************************************
 */

#ifndef _USER_CUSTS1_DEF_H_
#define _USER_CUSTS1_DEF_H_

#include "attm_db_128.h"

/*
 * SERVICE 1
 ****************************************************************************************
 */

#define DEF_SVC1_UUID_128 \
    {0x59, 0x5a, 0x08, 0xe4, 0x86, 0x2a, 0x9e, 0x8f, 0xe9, \
     0x11, 0xbc, 0x7c, 0x98, 0x43, 0x42, 0x18}

#define DEF_SVC1_CTRL_POINT_UUID_128 \
    {0x20, 0xEE, 0x8D, 0x0C, 0xE1, 0xF0, 0x4A, 0x0C, \
     0xB3, 0x25, 0xDC, 0x53, 0x6A, 0x68, 0x86, 0x2D}

#define DEF_SVC1_LED_STATE_UUID_128 \
    {0x4F, 0x43, 0x31, 0x3C, 0x93, 0x92, 0x42, 0xE6, \
     0xA8, 0x76, 0xFA, 0x3B, 0xEF, 0xB4, 0x87, 0x5A}

/* DO NOT CHANGE THIS BATTERY UUID */
#define DEF_SVC1_ADC_VAL_1_UUID_128 \
    {0x17, 0xB9, 0x67, 0x98, 0x4C, 0x66, 0x4C, 0x01, \
     0x96, 0x33, 0x31, 0xB1, 0x91, 0x59, 0x00, 0x15}

#define DEF_SVC1_ADC_VAL_2_UUID_128 \
    {0x23, 0x68, 0xEC, 0x52, 0x1E, 0x62, 0x44, 0x74, \
     0x9A, 0x1B, 0xD1, 0x8B, 0xAB, 0x75, 0xB6, 0x6E}

#define DEF_SVC1_BUTTON_STATE_UUID_128 \
    {0x9E, 0xE7, 0xBA, 0x08, 0xB9, 0xA9, 0x48, 0xAB, \
     0xA1, 0xAC, 0x03, 0x1C, 0x2E, 0x0D, 0x29, 0x6C}

#define DEF_SVC1_INDICATEABLE_UUID_128 \
    {0x28, 0xD5, 0xE1, 0xC1, 0xE1, 0xC5, 0x47, 0x29, \
     0xB5, 0x57, 0x65, 0xC3, 0xBA, 0x47, 0x15, 0x9E}

#define DEF_SVC1_LONG_VALUE_UUID_128 \
    {0x8C, 0x09, 0xE0, 0xD1, 0x81, 0x54, 0x42, 0x40, \
     0x8E, 0x4F, 0xD2, 0xB3, 0x77, 0xE3, 0x2A, 0x77}

#define DEF_SVC1_CTRL_POINT_CHAR_LEN     20
#define DEF_SVC1_LED_STATE_CHAR_LEN      20
#define DEF_SVC1_ADC_VAL_1_CHAR_LEN      1
#define DEF_SVC1_ADC_VAL_2_CHAR_LEN      2
#define DEF_SVC1_BUTTON_STATE_CHAR_LEN   1
#define DEF_SVC1_INDICATEABLE_CHAR_LEN   20
#define DEF_SVC1_LONG_VALUE_CHAR_LEN     50

#define DEF_SVC1_CONTROL_POINT_USER_DESC     "Control Point"
#define DEF_SVC1_LED_STATE_USER_DESC         "LED State"
#define DEF_SVC1_ADC_VAL_1_USER_DESC         "ADC Value 1"
#define DEF_SVC1_ADC_VAL_2_USER_DESC         "ADC Value 2"
#define DEF_SVC1_BUTTON_STATE_USER_DESC      "Button State"
#define DEF_SVC1_INDICATEABLE_USER_DESC      "Indicateable"
#define DEF_SVC1_LONG_VALUE_CHAR_USER_DESC   "Long Value"


/*
 * SERVICE 2
 ****************************************************************************************
 */

#define DEF_SVC2_UUID_128 \
    {0x59, 0x5a, 0x08, 0xe4, 0x86, 0x2a, 0x9e, 0x8f, 0xe9, \
     0x11, 0xbc, 0x7c, 0x7c, 0x46, 0x42, 0x18}

#define DEF_SVC2_WRITE_VAL_1_UUID_128 \
    {0x20, 0xEE, 0x8D, 0x0C, 0xE1, 0xF0, 0x4A, 0x0C, \
     0xB3, 0x25, 0xDC, 0x53, 0x6A, 0x68, 0x86, 0x2C}

#define DEF_SVC2_WRITE_VAL_2_UUID_128 \
    {0x4F, 0x43, 0x31, 0x3C, 0x93, 0x92, 0x42, 0xE6, \
     0xA8, 0x76, 0xFA, 0x3B, 0xEF, 0xB4, 0x87, 0x59}

#define DEF_SVC2_WRITE_VAL_1_CHAR_LEN    1
#define DEF_SVC2_WRITE_VAL_2_CHAR_LEN    1

#define DEF_SVC2_WRITE_VAL_1_USER_DESC   "Write me"
#define DEF_SVC2_WRITE_VAL_2_USER_DESC   "Write me (no rsp)"


/*
 * SERVICE 3
 ****************************************************************************************
 */

#define DEF_SVC3_UUID_128                 {0x59, 0x5a, 0x08, 0xe4, 0x86, 0x2a, 0x9e, 0x08, 0xe9,0x11, 0xbc, 0x7c, 0xd0, 0x47, 0x42, 0x18}

#define DEF_SVC3_READ_VAL_1_UUID_128      {0x17, 0xB9, 0x67, 0x98, 0x4C, 0x66, 0x4C, 0x01,0x96, 0x33, 0x31, 0xB1, 0x91, 0x59, 0x00, 0x14}

#define DEF_SVC3_READ_VAL_2_UUID_128      {0x23, 0x68, 0xEC, 0x52, 0x1E, 0x62, 0x44, 0x74,0x9A, 0x1B, 0xD1, 0x8B, 0xAB, 0x75, 0xB6, 0x6D}

#define DEF_SVC3_READ_VAL_3_UUID_128      {0x28, 0xD5, 0xE1, 0xC1, 0xE1, 0xC5, 0x47, 0x29,0xB5, 0x57, 0x65, 0xC3, 0xBA, 0x47, 0x15, 0x9D}

#define DEF_SVC3_READ_VAL_4_UUID_128      {0xF3, 0xD8, 0x1C, 0x45, 0x11, 0x96, 0x4D, 0x12, 0x13,0xFE, 0xD6, 0x3B, 0xCC, 0x31, 0x6C, 0x6E}


/*
 * HEART RATE
 *
 * Keep HR in Service 3 because this is the version that is
 * already working on your board.
 ****************************************************************************************
 */

#define DEF_SVC3_HR_VAL_UUID_128         {0x19, 0xB9, 0x67, 0x98, 0x4C, 0x66, 0x4C, 0x01,0x96, 0x33, 0x31, 0xB1, 0x91, 0x59, 0x00, 0x16}
		
/*
 * CLOCK
 ****************************************************************************************
 */

#define DEF_SVC3_CLOCK_VAL_UUID_128      {0x20, 0xB9, 0x67, 0x98, 0x4C, 0x66, 0x4C, 0x01,0x96, 0x33, 0x31, 0xB1, 0x91, 0x59, 0x00, 0x25}

#define DEF_SVC3_READ_VAL_1_CHAR_LEN     8
#define DEF_SVC3_READ_VAL_2_CHAR_LEN     2
#define DEF_SVC3_READ_VAL_3_CHAR_LEN     2
#define DEF_SVC3_READ_VAL_4_CHAR_LEN     2

#define DEF_SVC3_HR_VAL_CHAR_LEN         1		 
#define DEF_SVC3_CLOCK_VAL_CHAR_LEN      2

#define DEF_SVC3_READ_VAL_1_USER_DESC    "Sensor Data"
#define DEF_SVC3_READ_VAL_2_USER_DESC    "Read me"
#define DEF_SVC3_READ_VAL_3_USER_DESC    "Read me (indicate)"
#define DEF_SVC3_READ_VAL_4_USER_DESC    "Read me (RI)"
#define DEF_SVC3_HR_VAL_USER_DESC        "Heart Rate"
#define DEF_SVC3_CLOCK_VAL_USER_DESC     "Clock"

/* Sensor alias definitions */
#define DEF_SVC3_SENSOR_VAL_UUID_128     DEF_SVC3_READ_VAL_1_UUID_128
#define DEF_SVC3_SENSOR_VAL_CHAR_LEN     DEF_SVC3_READ_VAL_1_CHAR_LEN
#define DEF_SVC3_SENSOR_VAL_USER_DESC    DEF_SVC3_READ_VAL_1_USER_DESC

#define SVC3_IDX_SENSOR_VAL_CHAR         SVC3_IDX_READ_1_CHAR
#define SVC3_IDX_SENSOR_VAL_VAL          SVC3_IDX_READ_1_VAL
#define SVC3_IDX_SENSOR_VAL_NTF_CFG      SVC3_IDX_READ_1_NTF_CFG
#define SVC3_IDX_SENSOR_VAL_USER_DESC    SVC3_IDX_READ_1_USER_DESC


/*
 * DATABASE ENUM
 ****************************************************************************************
 */

enum
{
    /* Service 1 */
    SVC1_IDX_SVC = 0,

    SVC1_IDX_CONTROL_POINT_CHAR,
    SVC1_IDX_CONTROL_POINT_VAL,
    SVC1_IDX_CONTROL_POINT_USER_DESC,

    SVC1_IDX_LED_STATE_CHAR,
    SVC1_IDX_LED_STATE_VAL,
    SVC1_IDX_LED_STATE_USER_DESC,

    SVC1_IDX_ADC_VAL_1_CHAR,
    SVC1_IDX_ADC_VAL_1_VAL,
    SVC1_IDX_ADC_VAL_1_NTF_CFG,
    SVC1_IDX_ADC_VAL_1_USER_DESC,

    SVC1_IDX_ADC_VAL_2_CHAR,
    SVC1_IDX_ADC_VAL_2_VAL,
    SVC1_IDX_ADC_VAL_2_USER_DESC,

    SVC1_IDX_BUTTON_STATE_CHAR,
    SVC1_IDX_BUTTON_STATE_VAL,
    SVC1_IDX_BUTTON_STATE_NTF_CFG,
    SVC1_IDX_BUTTON_STATE_USER_DESC,

    SVC1_IDX_INDICATEABLE_CHAR,
    SVC1_IDX_INDICATEABLE_VAL,
    SVC1_IDX_INDICATEABLE_IND_CFG,
    SVC1_IDX_INDICATEABLE_USER_DESC,

    SVC1_IDX_LONG_VALUE_CHAR,
    SVC1_IDX_LONG_VALUE_VAL,
    SVC1_IDX_LONG_VALUE_NTF_CFG,
    SVC1_IDX_LONG_VALUE_USER_DESC,


    /* Service 2 */
    SVC2_IDX_SVC,

    SVC2_WRITE_1_CHAR,
    SVC2_WRITE_1_VAL,
    SVC2_WRITE_1_USER_DESC,

    SVC2_WRITE_2_CHAR,
    SVC2_WRITE_2_VAL,
    SVC2_WRITE_2_USER_DESC,


    /* Service 3 */
    SVC3_IDX_SVC,

    SVC3_IDX_READ_1_CHAR,
    SVC3_IDX_READ_1_VAL,
    SVC3_IDX_READ_1_NTF_CFG,
    SVC3_IDX_READ_1_USER_DESC,

    SVC3_IDX_READ_2_CHAR,
    SVC3_IDX_READ_2_VAL,
    SVC3_IDX_READ_2_USER_DESC,

    SVC3_IDX_READ_3_CHAR,
    SVC3_IDX_READ_3_VAL,
    SVC3_IDX_READ_3_IND_CFG,
    SVC3_IDX_READ_3_USER_DESC,

    SVC3_IDX_READ_4_CHAR,
    SVC3_IDX_READ_4_VAL,
    SVC3_IDX_READ_4_USER_DESC,

    /* Heart Rate */
    SVC3_IDX_HR_VAL_CHAR,
    SVC3_IDX_HR_VAL_VAL,
    SVC3_IDX_HR_VAL_NTF_CFG,
    SVC3_IDX_HR_VAL_USER_DESC,
		
		/* Clock */
    SVC3_IDX_CLOCK_VAL_CHAR,
    SVC3_IDX_CLOCK_VAL_VAL,
    SVC3_IDX_CLOCK_VAL_USER_DESC,

    CUSTS1_IDX_NB
};

#endif