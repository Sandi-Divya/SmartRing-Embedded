...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWIP__)
typedef uint8_t (*check_sys_startup_period_t)(void);
uint32_t rwip_sleep_step;
extern uint8_t check_sys_startup_period(void);
extern bool rwip_sleep_part1_ROM(uint32_t *rwip_sleep_step, sleep_mode_t *ptr_proc_sleep, check_sys_startup_period_t func);
extern bool rwip_sleep_part2_ROM(uint32_t *rwip_sleep_step, uint32_t *sleep_duration);
extern bool rwip_sleep_part3_ROM(uint32_t *sleep_duration);
#endif // __EXCLUDE_ROM_RWIP__
...
static uint32_t rwip_slot_2_lpcycles_rcx(uint32_t slot_cnt)
{
    ...
    lpcycles = (uint32_t) ((slot_cnt * rcx_time_data.rcx_slot_duration) >> 20);

    return lpcycles;
}

static uint32_t get_max_sleep_duration(void)
{
    ...
    {
        if (EXTERNAL_WAKEUP || UART_FLOW_CTRL_WAKEUP)
        {
...
sleep_mode_t rwip_sleep(void)
{
    ...
    else if (!EXTERNAL_WAKEUP || !UART_FLOW_CTRL_WAKEUP) // sleep_duration will remain as it was set above....
    {
    ...
    {
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWIP__)
        if (rwip_sleep_part1_ROM(&rwip_sleep_step, &proc_sleep, check_sys_startup_period))
        {
            break;
        }
#else
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWIP__)
    if (rwip_sleep_part2_ROM(&rwip_sleep_step, &sleep_duration))
    {
        break;
    }
#else
        ...
        {
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWIP__)
            if (rwip_sleep_part3_ROM(&sleep_duration))
            {
                break;
            }
#else
        ...
        if ( arch_clk_is_RCX20() )
        {
            ...
            if (GetBits16(ANA_STATUS_REG, BOOST_SELECTED) == 1)
                ...
                uint32_t lpcycles = lld_sleep_us_2_lpcycles_sel_func(LP_ISR_TIME_USEC + XTAL_HIGH_TEMP_LATENCY_STARTUP_USEC);
                ...
                uint32_t xtal_settling_cycles = lld_sleep_us_2_lpcycles_sel_func(XTAL16M_SETTLING_IN_USEC + XTAL_HIGH_TEMP_LATENCY_STARTUP_USEC);
        ...
        else if ( arch_clk_is_XTAL32( ) )
        {
                ...
                #else
                    // Set TWIRQ_SET taking into account that some LP cycles are needed for the power up FSM.
                    twirq_set_value = XTAL32_POWER_UP_TIME + LP_ISR_TIME_XTAL32_CYCLES + XTAL_HIGH_TEMP_LATENCY_STARTUP_XTAL32_CYCLES;
                #endif
                ...
                #else
                    // Program LP deassertion to occur when the XTAL16M has settled
                    twirq_reset_value = LP_ISR_TIME_XTAL32_CYCLES + XTAL_HIGH_TEMP_LATENCY_STARTUP_XTAL32_CYCLES - XTAL16M_SETTLING_IN_XTAL32_CYCLES;
                #endif

                // TWEXT setting
                twext_value = LP_ISR_TIME_XTAL32_CYCLES + XTAL_HIGH_TEMP_LATENCY_STARTUP_XTAL32_CYCLES;
            #endif
        }
#endif // __DA14531__
